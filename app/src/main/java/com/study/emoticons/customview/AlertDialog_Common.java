 ...
 * context 上下文对象
 * dialogView 自定义布局的View对象,提供被外界访问的接口get和set方法,并且利用自定义的监听器将view对象回调出去
 * listener 监听器　用于将确定和取消意图的两个点击事件监听器，合成一个监听器，并传入一个标记变量区别确定和取消意图
 */
public class AlertDialog_Common {
    ...
    //带有自定义view的构造器
    public AlertDialog_Common(Context context, int customeLayoutId, String dialogTitle, String positiveText, String negativeText) {
    ...
    //不带自定义view的构造器
    public AlertDialog_Common(Context context, String dialogMessage, String dialogTitle, String positiveText, String negativeText) {
        this.context = context;
        this.dialogTitle = dialogTitle;
    ...
    public void showDialog(){
        AlertDialog_Custom.Builder dialog=new AlertDialog_Custom.Builder(context);
    ...
    //注册监听器方法
    public AlertDialog_Common setOnDiaLogListener(OnDialogListener listener){
